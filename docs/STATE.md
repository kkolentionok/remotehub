...
## Latest — Fix: HTTPS fails behind corporate TLS inspection

Claiming a code on a corporate machine died with `error sending request`. Diagnosis from the machine itself: DNS resolved correctly, `Test-NetConnection … -Port 443` succeeded, yet `curl https://ya.ru` also returned `000` — every HTTPS request failed while the browser worked. That signature is TLS interception: the TCP connection is fine, the certificate is replaced by a corporate CA, and a client that doesn't trust that CA gives up at the handshake.

We were building reqwest with `rustls-tls`, which trusts **only the bundled Mozilla root list** and ignores the Windows certificate store — so on such a network nothing of ours could ever connect, notes or vault alike. Switched to `rustls-tls-native-roots`, which keeps rustls (no OpenSSL, no extra build step) but trusts the same CAs the OS does. This is the right default for a desktop app regardless of this particular network.

Also: `sync_remote`'s pairing helper now reports `unreachable` for connect/timeout failures instead of echoing reqwest's internals, and the claim screen renders that as a sentence about the network and corporate proxies (`tools.notes.claim.offline`, en+ru).

`tsc --noEmit` + `vite build` green. **Rust compiled by the user** — the Cargo feature change means a fresh dependency build.

## Latest — Batch B2b: the access code, end to end

The feature this whole thread was for. A signed-in device mints a code; a device with no account redeems it and gets notes, and only notes.

- **`rh-vault`**: `gen_pairing_code` (8 Crockford base32 symbols — no I/L/O/U, the characters people misread when reading aloud — ≈40 bits), `normalize_code` (so `k7qd-m2xr`, `K7QD M2XR` and `K7QDM2XR` are one code), `code_hash` (SHA-256, the only thing the server ever sees) and `code_key` (Argon2id with a salt derived from the code, so both devices reach the same key knowing only the code). `sha2` added to the crate.
- **`rh-app`**: `notes_pair_create` wraps the notes key under the code key and parks it at `POST /v1/pair`; `notes_pair_claim` redeems it, unwraps locally — a wrong code fails at the AEAD tag, not at the server — and stores the notes key plus the notes-scoped token; `notes_pair_devices` / `notes_pair_revoke` / `notes_mode` / `notes_unpair`. HTTP helpers in `sync_remote`.
- **UI** (`NotesPairing.tsx` + module): a KeyRound trigger in the notes list header (hidden on a paired device — the code grants access to all notes, so it belongs to the list, not to a single note's toolbar), and a popover rendered through a **portal** because the list clips overflow. Code in large mono, click to copy, quiet countdown, and the paired-device list with inline revoke confirmation. An expired code is struck through and replaced by "generate a new one" rather than silently swapped — the user may be reading the old one out loud.
- **Claim screen**: a device that is neither signed in nor paired gets the code field instead of the notes UI. Auto-uppercase, auto-hyphen, paste-whole-code friendly. Once paired, the header reads "Notes only".

Deliberately deferred: hiding Storage and Tools app-wide in notes-only mode (B3). The mode is a runtime state, not a separate build, so the portable `light.exe` and the installed build behave identically — what differs is whether an account is signed in.

`tsc --noEmit` + `vite build` green. **Rust compiled by the user.** Requires the B1 server (already deployed).

## Latest — Fix: the first-run notes key race (notes not arriving, notes lost)

Reported straight after B2a: a note typed on one PC never reached the other, and Refresh didn't help. One root cause, three bad rules of mine.

Both devices minted their own notes key on their first notes pass. The vault merge picked one — but `apply_snapshot` **refused to overwrite an existing local key**, so the loser kept its own forever. It could no longer open the shared blob, `open_notes` returned an error, and the pass bailed out *before pushing*, so that device could neither read nor write: a permanent dead end. Meanwhile whichever device did push overwrote the other's blob, which is where the missing notes went.

- **The merged key is authoritative.** `apply_snapshot` now adopts `notes_key_b64` whenever it differs from the local one. The old "never overwrite, or paired devices get orphaned" reasoning was backwards — pairing happens after convergence, and disagreeing with the converged value means being unable to read anything.
- **One minting site.** Keys are minted only in `build_snapshot`, so every key goes through `merge` (which resolves a collision deterministically) before any notes traffic. A notes pass with no key now does nothing instead of sealing a blob nobody else can open.
- **Self-healing.** If the remote blob can't be opened with the current key, the pass logs it and republishes local notes under the current key rather than erroring out forever.
- `fingerprint` now covers `notes_key_b64`, which lives outside `records` — otherwise the idle short-circuit could skip the very pass that would publish or adopt a key.
- Added a debug line with the first bytes of the key in use, so a mismatch is visible in the log rather than inferred.

Notes lost in the earlier overwrite are not recoverable — the blob was replaced, not merged.

**Rust compiled by the user.**

## Latest — Batch B2a: notes off the vault path, onto their own blob and loop

The client half of the split. Notes no longer travel inside the vault snapshot at all.

- **`rh-vault`**: `VaultKey::from_bytes`; new `notes_blob` module — `seal_notes`/`open_notes` (minimal envelope: format, random nonce, ciphertext — no KDF params because there is no KDF), `gen_notes_key`, and `wrap_notes_key`/`unwrap_notes_key` for the pairing handshake. `SyncSnapshot` gains `notes_key_b64` (`#[serde(default)]`), and `merge` carries it: first side to have a key wins, and if both minted one the lexicographically smaller is taken so two racing devices converge instead of flip-flopping.
- **`rh-app/notes_sync.rs`**: builds a notes-only snapshot (live records + `KIND_NOTE` tombstones), pulls `/v1/notes`, merges with the same HLC rules, pushes only when the merge differs from what the server holds, and applies under the same `local_is_current` provenance guard the vault path uses.
- **Key distribution**: the notes key is random, lives in the OS keychain (`notes-key`), and rides inside the already-sealed vault snapshot so a second signed-in device adopts it automatically. `apply_snapshot` never *overwrites* an existing local key — doing so would orphan every device already paired against it. `wipe_local` clears the key and any notes token.
- **`api/vault.rs`**: note records and note tombstones removed from `build_snapshot`/`apply_snapshot`. Existing notes need no migration — they are already in local SQLite, so the first notes pass simply populates the new blob from them.
- **Loops split** (`sync_engine`): the vault is back to a flat **30 s** cadence, and `run_notes_loop` runs notes at **1 s** while their screen is open, 15 s otherwise, woken by `AppState.notes_wake` on every note mutation. Coupling the two is what made notes feel slow — delivering a note in a second meant rebuilding, re-deriving and re-applying the entire vault every second. A notes pass is now a small GET plus one AES-GCM open: no Argon2, no keychain sweep.
- `ServerRemote` gained a `path`, so one transport serves `/v1/vault` and `/v1/notes`; `sync_refresh` flushes notes first, since the button that calls it sits on the notes screen.

Next (B2b): the code button and popover — `wrap_notes_key` + `POST /v1/pair` on the owner's side, the claim screen and notes-only mode on the other.

No `ui/` change in this slice. **Rust compiled by the user** — this is the widest slice so far (rh-vault public surface, vault snapshot shape, a new module).

## Latest — Batch B1: notes get their own blob on the server + pairing by code

Server-only slice (`server/`, needs a redeploy before the client half works). Notes move out of the vault blob because scoped access is impossible while hosts, credentials and notes share one opaque envelope — that constraint is what the access code was blocked on all along.

- **Schema** (`db.rs`, all `IF NOT EXISTS`): `notes_blobs` (one sealed blob per account, `rev` for optimistic concurrency — same contract as `vaults`); `pairings` (`code_hash` PK, `wrapped_key`, `expires_at`, `claimed_at`); `notes_devices` (paired devices, `revoked` flag, `last_seen_at`).
- **`notes.rs`**: `GET/PUT /v1/notes` behind a new `NotesAccess` extractor that accepts either a full account token or a **notes-scoped** one. `POST /v1/pair` (owner registers a pending pairing), `POST /v1/pair/claim` (unauthenticated — the code *is* the credential), `GET /v1/pair/devices`, `DELETE /v1/pair/devices/:id`.
- **`auth.rs`**: `Claims.did`, `issue_notes_token` (`typ = "notes"`, carries the device id), `decode_claims`. A notes token is rejected by every vault route, and the extractor re-reads `revoked` on **every** request, so revocation is immediate rather than waiting out an expiry.
- **What the server can't do**: the code never reaches it — the client sends only a hash — so the `wrapped_key` it stores is un-unwrappable by the server. Claiming is a single conditional UPDATE, so it is atomic, single-use and expiry-checked in one statement, and wrong/used/expired all return the same 401 so a guesser learns nothing. 8 Crockford-base32 chars ≈ 40 bits with a 10-minute single-use window.
- `AppError::NotFound` added (revoking an unknown device).

Next (B2, client): notes key generation + keychain storage, notes moved off the vault path onto their own light sync loop, the code button and popover, then B3 for the notes-only mode in the portable build.

No `ui/` change in this slice, so no frontend rebuild. **Server compiled by the user via Docker.**

## Latest — Sync latency: the fresh-salt-per-push trap, and a cached vault key

Notes still arrived slowly after the idle short-circuit, so the remaining cost got measured properly. It was the KDF, and it was self-inflicted.

`seal_snapshot` minted a **fresh Argon2id salt on every push**. New salt ⇒ new key ⇒ the sending device derived twice per pass (open, then seal), and every *other* device's key was invalidated by every single edit, so it derived again on receipt. Three derivations at 64 MiB / t=3 per round trip — roughly a second of pure CPU, paid on a timer.

- **`rh-vault`**: `open_envelope_with_key` / `seal_snapshot_with_key` (key-taking variants; the password-taking ones now delegate). New `keycache` module — `KeyCache` holds one derived key keyed by its `KdfParams`, so different parameters or a different account miss and re-derive rather than misapply. `KdfParams` derives `PartialEq`/`Eq`. `sync_once_cached` takes a cache and, crucially, **re-seals under the KDF params the pulled blob already carried** instead of generating a new salt. Nonces remain freshly random per seal, so nothing about AES-GCM's uniqueness requirement changes — only the salt is held stable, which is how a vault's own salt is meant to behave. `sync_once` stays as a wrapper over a throwaway cache.
- **`rh-app`**: `AppState.vault_keys`; `run_sync_core` uses `sync_once_cached`. Cleared on `sync_set_master` (a new password must not reuse the old key) and on logout, along with `sync_seen` — both are account-scoped.
- **Cadence**: fast tick 2 s → **1 s**, push debounce 400 ms → 250 ms, now that a pass costs a small GET and (usually) no derivation at all.
- **Typing nudge** (user's suggestion): the editor fires one `sync.refresh()` when typing resumes after a 10 s lull, so the first keystroke of a hand-over pulls the other side's latest immediately. Rate-limited to once per 10 s — a nudge per keystroke would be a self-inflicted DoS.

Remaining known cost per pass: `build_snapshot` still reveals every credential secret from the OS keychain. Batch B (notes in their own small blob with their own key) removes the whole vault from the notes path.

`tsc --noEmit` + `vite build` green. **Rust compiled by the user** — this touches `rh-vault`'s public surface, so a stale call site would surface at compile time.

## Latest — Deleted notes came back (root cause) + sync made cheap + manual refresh

**The bug.** Deleting a note on one device brought it straight back. Not a merge flaw — a *window* inside a pass. `run_sync_core` builds the local snapshot, then spends network + two Argon2id derivations before `apply_snapshot` writes the merge back. A delete landing inside that window is invisible to the snapshot that was already built, so the merge still carries the note as live; apply re-created the row and — the fatal part — `sync_meta.bump()` cleared the tombstone (an UPSERT with `deleted = 0`). The deletion was gone for good and the resurrected note was pushed to every device. At the 30 s cadence the window was rarely hit; at the notes cadence it was hit constantly.

**Fix**: `apply_snapshot` now consults local provenance before touching anything — `local_is_current(state, kind, id, rec)` skips a record when the local `sync_meta` stamp is equal to it (already applied) or wins over it (a local edit/delete landed later). Applied to every upsert phase *and* every deletion phase. The tombstone survives and propagates on the next pass. This is a general fix: hosts/groups/credentials had the same race, just rarely enough to go unnoticed.

**Speed.** A pass was doing far too much to repeat every few seconds: build the whole snapshot (revealing every credential from the keychain), GET, Argon2id 64 MiB to open, merge, Argon2id again to seal, PUT, then rewrite every record back into SQLite and the keychain. Two changes:
- The provenance guard above makes an unchanged record a no-op instead of a full rewrite — the apply phase now costs nothing when nothing moved.
- `run_sync_core` gained an idle short-circuit: `AppState.sync_seen` remembers `(remote version, fingerprint of the applied records)`; if our records fingerprint the same and the server is still on that version, the pass returns after one small GET — **before any key derivation**. Fingerprint is order-independent (records serialized, sorted, hashed) so `build_snapshot`'s append order and `merge`'s sorted output agree.
- Fast cadence 3 s → **2 s** now that an idle pass is nearly free.

**Manual refresh**: `sync_refresh` command (runs one pass via `sync_engine::run_pass`, so status events fire as usual) + a RefreshCw button in the notes list header that spins while in flight and re-reads the open note afterwards. `sync.refresh()` in `ipc.ts`, `tools.notes.refresh` en+ru.

Still architectural, not fixed here: every pass re-derives the vault key and re-reads every credential secret. Batch B moves notes into their own small blob with its own key, which removes both from the notes path entirely.

`tsc --noEmit` + `vite build` green. **Rust compiled by the user.**

## Latest — Notes screen to the approved design + pinning

Designer handoff ported onto the real screen (`rhub__3_.zip`, three mockup frames). The oversized header with its description is gone — the tab now opens straight into list + editor at SFTP density.
- **Pinning** (new field): `Note.pinned` (`#[serde(default)]` so older snapshots still decode), migration **v14** (`ALTER TABLE notes ADD COLUMN pinned`), `NoteStore::set_pinned`, `note_set_pinned` command (persists immediately, like the other boolean toggles), list ordered `pinned DESC, updated_at DESC`.
- **List**: search filter, sticky hairline group headers — Pinned / Today / Yesterday / Earlier — accent left-edge on the selected row, mono timestamp + first-line preview, distinct empty states for "no notes" and "nothing matches".
- **Editor**: quiet inline save indicator (11px text + check, `saving…` only while in flight — deliberately *not* `SaveStatusIndicator`, which is louder than this typing flow wants), pin / copy / delete, and a disabled B/I/U/S + lists + attach bar so the header doesn't need re-laying out when rich text lands.
- **Dropped from the mockup**: the "changed on another device" dot and its inline notice — the user chose not to track this at all. The list instead refreshes on a 2 s poll of the local store, which never clobbers an in-progress edit (`dirty` ref).
- i18n en+ru for every new string; tokens only (the handoff's literal `Geist Mono` and half-pixel sizes normalized onto `--font-mono` and the project scale).

Still to come (batch B): the access-code button, which needs notes moved into their own encrypted blob + server-side pairing, scoped token and device revoke.

`tsc --noEmit` + `vite build` green. **Rust compiled by the user** — schema is now **v14**.
...
- **rh-core**: `ForwardId` ULID newtype; `ForwardKind` MOVED here (was in rh-ssh; rh-ssh now `pub use rh_core::ForwardKind` so existing paths hold) + `as_str`/`from_tag`; `SavedForward` struct; `ForwardStore` trait (create/get/list/delete/set_auto_start).
- **rh-storage**: `forward_store.rs` (`SqliteForwardStore`); migration **v11** `forwards` table (id, host_id FK cascade, kind, bind_host/port, target_host/port, auto_start, created_at) + index; `CURRENT_SCHEMA_VERSION=11`; v1.sql updated.
- **rh-app**: `AppState.forward_defs: Arc<dyn ForwardStore>`. Commands replaced: `forward_save` (persist def), `forward_start` (run a saved def; no-op if already live), `forward_stop` (stop, keep def), `forward_delete` (stop + delete), `forward_set_auto_start`, `forward_list` (saved defs annotated with live `state`/`active`, `state=null` when not running). `ForwardManager` slimmed: `register(id, handle, join, rx, Option<Channel>)` + `state_of`/`is_live` (def now lives in the store, not the entry). **Auto-start at launch** via `autostart_all` spawned in `main.rs setup` (best-effort, no UI channel).
- **ui**: `ForwardSaved`/`ForwardSaveRequest`/`ForwardSaveResponse` types; `forwards.save/start/stop/delete/setAutoStart/list`. ForwardsPane list now shows **saved** forwards with per-row Run/Stop, **auto-start toggle** (Zap, lit when on), and Delete; the form's button is **Add** (save + start). Form layout unchanged (redesign still parked). i18n add/run/delete/autoStart/saved/deleted/state.stopped (en+ru).

strict TOFU still deferred (forwards reuse the bastion-style trusting handler). `tsc -b` + `vite build` green. **Rust compiled by user.** Built on a clean `git reset --hard origin/main` (7bb73cf, which already carried a single `ForwardId` in id.rs — NOT re-added, that was the earlier duplicate-definition bug) so the archive is a true superset, no slice-2 regression.

## Latest — Forwards form: symmetric host:port ⇄ host:port pairs

Per user feedback the form now shows **two symmetric `host:port` pairs** around the arrow — `[SSH host ▾] › [host]:[port] ⇄ [host]:[port] [Start]` — instead of the previous `host:port ⇄ port` (the right side was a bare port). Added an editable **bind host** input (defaults `127.0.0.1`; `bind_host` sent in `ForwardOpenRequest`, already supported by the backend). Dynamic shows just the local SOCKS pair. Live-row text matches: `target:port ⇄ bind_host:bind_port` (dynamic: `SOCKS5 bind_host:bind_port`). Placeholders per kind (remote host / local host / server bind / local|server port). The remaining per-kind side-semantics inversion is documented in ROADMAP as minor debt for the persist-UI pass. `tsc -b` + `vite build` green. Frontend only.

## Latest — Port forwarding slice 2: remote -R, dynamic -D (SOCKS5) + form rework

Added the remaining SSH forward directions on top of slice-1 `-L`:
- **`-R` remote** — `rh-ssh`: `ClientHandler` got a `forwarded_tx` sink + `with_forwarded_sink()` builder + the `server_channel_open_forwarded_tcpip` callback (signature verified against the russh 0.45 crate source). `forward.rs` `run_remote` calls `handle.tcpip_forward()`, then bridges each server-opened channel to a locally-reachable `target` via `TcpStream::connect`; `cancel_tcpip_forward` on stop.
- **`-D` dynamic** — `forward.rs` `run_listener` + a minimal SOCKS5 CONNECT handshake (`socks5_accept`): no-auth greeting, IPv4/IPv6/domain, per-connection `channel_open_direct_tcpip` to the address the client asked for.
- DTO `ForwardOpenRequest` gained `kind` (default `local`), `target_*` now optional (unused for `-D`); `ForwardKind` derives `Deserialize`. UI: segmented Local/Remote/Dynamic selector + per-kind labels/hints, kind tag on live rows.

**Form rework (user feedback):** the field order didn't match the mental model and `127.0.0.1` was noise. New flow reads **`[Host] › [remote host]:[port] ⇄ [local port] [Start]`** — destination first, local port last (per "удалённый хост, удалённый порт → получить на свой порт"). Dropped `127.0.0.1` from the form and from the live-row text (now `:{bind} → {target}:{port}`). **Bug fix:** a forward whose async bind/connect failed (e.g. local port 22 → `os error 10013`, a privileged/blocked port on Windows) used to vanish silently on the next refresh; the `error` ForwardEvent is now surfaced as a toast.

`tsc -b` + `vite build` green. **Rust unverified (user compiles).** Risk: `-R` russh callback + `tcpip_forward`/`cancel_tcpip_forward` + the hand-written SOCKS5 are new/unproven. **Next:** persist forward definitions (DB migration + store + UI) + strict TOFU.

## Latest — Ctrl/Cmd+K palette now opens from the terminal too (+ layout-independent)

User reported Ctrl+K did nothing in the terminal — it was **by design** (we let the shell keep Ctrl+K kill-line, so on an empty prompt nothing visible happened). Reversed that: the command palette is the marquee global shortcut, so it should open everywhere. `AppShell` now listens for Ctrl/Cmd+K in the **capture phase** and `stopImmediatePropagation()`s, so xterm never sees the key and doesn't also send the shell's kill-line (`\x0b`). **Trade-off:** Ctrl+K kill-line in the shell is now overridden by the palette; **Ctrl+U** (kill line) / **Ctrl+W** (kill word) still reach the shell. If kill-line is wanted back, revert this `useEffect` to the `.xterm` early-return.

Follow-up: didn't fire on a **Russian layout** — was matching `e.key.toLowerCase() === "k"`, but physical K under ЙЦУКЕН yields `e.key === "л"`. Switched to `e.code === "KeyK"` (physical key, layout-independent), matching how the Ctrl+Shift+E/D split shortcuts already work. The `?` shortcut is fine (char-based, reachable on both layouts). Frontend only, `tsc -b` + `vite build` green.

## Latest — "Broken fonts" root cause = RDP client, not CSS → logged to roadmap

User clarified the long-running "резкие/некрасивые шрифты" complaint: he runs the dev build **on a laptop and views it over RDP**. Text looks harsh **only through RemoteHub's own RDP client**; via `mstsc` (after restarting `cargo tauri dev`) it's smooth; reconnect via RemoteHub → harsh again. So it was **never** a CSS/font issue — it's how RemoteHub's RDP client renders the remote frame (DPI / scaling / resampling). All earlier CSS-font experiments stay reverted; **do not touch CSS fonts for this symptom.** Also surfaced: typing `,` shows `.` over RemoteHub RDP — keyboard scancode/layout bug in `rh-rdp`.

Both captured **in detail in `docs/ROADMAP.md` → RDP features** (text-sharpness/DPI bug + comma→period keyboard bug), each with full repro, hypothesis, what-to-check, and open questions. **Not fixing now** — user: "сегодня RDP не трогаем, пофиксим потом." Docs only this turn.

## Latest — Command palette (Ctrl/Cmd+K) — hotkeys slice 2a

Generalized the existing host quick-connect `Launcher` into a real command palette (frontend only). Opened by **Ctrl/Cmd+K** (new), the tab-bar **"+"**, or a pane split:
- **Command mode** (Ctrl/Cmd+K / "+", i.e. no pending split): two fuzzy-filtered sections — **Actions** (New host, New group, Manage credentials, Port forwarding, Settings, Keyboard shortcuts — each wired to existing `uiStore` setters / `setDialog`) and **Hosts** (connect, SSH-only as before). Arrow/Enter nav across the combined list, Esc closes.
- **Split mode** (`splitTarget` set): unchanged — hosts only, chosen host drops into the new pane.
- **Ctrl/Cmd+K handler** in `AppShell` is suppressed while the terminal (`.xterm`) is focused, so it never steals the shell's Ctrl+K (kill-line); forces command mode (`splitTarget=null`) then opens.
- `ShortcutsSheet` now lists Ctrl/Cmd+K (Command palette). i18n `palette.*` + `shortcuts.row.palette` (en+ru). RDP rows still disabled in connect (RDP path untouched).

`tsc -b` + `vite build` green. **Next (2b):** J/K sidebar navigation.

## Latest — Forwards form polish

**Forwards form, more logical** (`ToolsView` ForwardsPane): host combobox now labels each host as **`name [address]`** (e.g. `richard-tea.com [92.63.193.34]`; collapses to just the address when name == hostname). Dropped the confusing static `127.0.0.1` label. New left-to-right flow: **`Хост [IP] › [порт] ⇄ localhost : [порт] [Старт]`** (`ChevronRight` separates host→local-port; `ArrowRightLeft` is the tunnel). Bind still defaults to 127.0.0.1 on the backend; the live list row still shows `127.0.0.1:<port>` since that's where you connect. `tsc -b` + `vite build` green. Frontend only.

*(Note: a font/terminal-renderer experiment to soften Windows text — switching to OS-native fonts + the DOM terminal renderer — was tried and **fully reverted** at the user's request. Fonts are back to Geist-first and the terminal back to the WebGL renderer, exactly as before.)*

## Latest — Port forwarding (local `-L`, slice 1)

SSH **local port forwarding** as a standalone Tools feature (Tools → Проброс портов). A forward is its own unit keyed to a saved SSH host: it opens its **own** SSH connection (independent of any terminal), binds a local `TcpListener`, and per accepted conn opens a `direct-tcpip` channel + `copy_bidirectional`. Reuses the **exact** proven russh surface the ProxyJump bastion already uses, so the only new code is the accept loop + the manager/IPC/UI plumbing.

- **`crates/rh-ssh/src/forward.rs`** (new) — `spawn_forward(params, events) → (ForwardHandle, JoinHandle)`. Connect+auth reuse `ClientHandler::trusting` + `try_all_auth` + `CONNECT_TIMEOUT` (exposed `pub(crate)` in `actor.rs`); host key auto-accepted like the bastion (forward targets a host the user has typically already pinned). Public types `ForwardKind(Local)`, `ForwardSpec`, `ForwardState`, `ForwardEvent` (StateChanged/ActiveChanged/Error/Closed), `ForwardConnectParams`, `ForwardSpawnParams`, `ForwardHandle{shutdown()}`. Accept loop `select!`s on a cancel `Notify` + shutdown flag; per conn `channel_open_direct_tcpip(target,port,"127.0.0.1",0)` → `into_stream()` → `tokio::io::copy_bidirectional`; live count via `AtomicU32` → `ActiveChanged`. One-level ProxyJump supported (bastion kept alive for the forward's life).
- **`crates/rh-app`** — `forward_session.rs` (new) `ForwardManager` (registry + event pump → UI `Channel` + supervisor evict-on-exit; in-memory only). `api/forwards.rs` (new) `forward_open` / `forward_close` / `forward_list`; `forward_open` resolves host (SSH-only) + creds via `sessions::revealed_creds_for` + same one-level jump resolution as `session_open`. DTOs in `api/dto.rs`. Wired into `state.rs` (`forwards: ForwardManager`), `main.rs` (`mod forward_session;` + 3 commands), `api/mod.rs`.
- **Tray** — new **"Port forwarding"** item (after Open) → shows the window + emits `tray:open-forwards`; `AppShell` listens → Tools section + `uiStore.toolsSection="forwards"`; `ToolsView` consumes `toolsSection` to land on the pane. *(User asked for a tray entry worded "Примонтированные диски" — that's the future **mounts** feature which isn't built; for this slice the tray opens Tools→Forwards. Easy to relabel/repoint when mounts land.)*
- **Frontend** — `lib/types.ts` + `lib/ipc.ts` (`forwards.open/close/list`, Channel pattern) + `ToolsView` `ForwardsPane` (host Combobox + `127.0.0.1:port → target:port` form + Start; live list with state chip / active count / Stop; polls `forward_list` every 2s so reload-orphaned forwards still show). `forwards` nav flipped `soon:false`. i18n `tools.forwards.*` (en+ru). `tsc -b` + `vite build` green.
- **Flags:** Rust **unverified — user compiles** (only new russh code is the accept loop; tokio `net`/`io-util`/`macros` confirmed available to rh-ssh). **Deferred to slice 2:** remote `-R`, dynamic SOCKS `-D`, DB persistence of forward definitions, strict TOFU prompt for forwards.

## Latest — Keyboard-shortcuts cheat sheet (`?`)

New `?` overlay listing the app's *real, working* shortcuts, grouped General / Terminal / SFTP. Pure frontend; backend untouched.
- **`components/layout/ShortcutsSheet.tsx`** (+ `.module.css`) — backdrop + centered panel (tokens + `--shadow-pop`); each shortcut's keys render as adjacent `<kbd>` chips (chord = chips side by side, Raycast/Linear-style). Esc or backdrop-click closes.
- **`AppShell`** — a global `?` keydown toggles it, guarded against input/textarea/contenteditable focus (incl. xterm's helper textarea) and any held modifier, so it can never hijack a typed "?". Rendered alongside `Launcher`.
- **`uiStore`** — `shortcutsOpen` + `setShortcutsOpen` (mirrors `launcherOpen`).
- **i18n** `shortcuts.*` (en+ru). **Honest list only:** Ctrl/Cmd+K is deliberately absent (the old `CommandBar` is never mounted → that binding is dead); RDP shortcuts deliberately omitted. Listed: Esc, Ctrl+Shift+E / Ctrl+Shift+D (split), ↑↓+Enter (list nav), Ctrl+scroll (terminal zoom), Ctrl/Cmd+click (terminal link / SFTP multi-select), Enter/Esc (SFTP inline edit), `?` (this sheet).
- Seam for the future hotkeys/command-palette backlog item. `tsc -b` + `vite build` green.

## Latest — RDP floating bar ("shield"): draggable + Minimize + Close-to-tab

Upgraded the floating session bar in `RdpViewport.tsx`. (1) **Draggable**: grab it by the host name (pointer drag) and drop anywhere; it snaps to the nearest perimeter anchor (3 cols × 2 rows → tl/tc/tr/bl/bc/br). Placement is component state via `.pos_*` classes; live drag uses inline left/top. (2) **Minimize** button (`Minus` icon, `session.minimize`) — pop-out only. (3) **Close-to-tab** button (`X`, red hover, `session.redock`) — pop-out only; returns the session to a main-window tab instead of killing it. New props `onMinimize` / `onPopIn` gate the two new buttons (mirrors the `onPopOut` pattern, so the inline in-app view is unchanged). `RdpPopoutApp` wires `onMinimize → getCurrentWindow().minimize()` and `onPopIn → emit("rdp:request-redock",{sid})`; the main window listens for `rdp:request-redock` (alongside `rdp:popout-closed`, same one-time guard) and calls the existing `redockRdp(owner.key)`. Added i18n `session.moveBar` (en+ru); reused existing `session.minimize`/`session.redock`. Frontend verified (tsc + vite green). Pure frontend; backend untouched.

## Latest — Fix: pop-out "return to tab" (X) hung the window

Bug: the X in a popped-out RDP window didn't close it and didn't re-dock — the window just hung. Root cause was a fragile cross-window dance: the pop-out emitted `rdp:request-redock`, then the **main** window closed the pop-out via `WebviewWindow.getByLabel().close()` (an `await` that could hang/race), and `reattach` sat outside the try.

Fix: the pop-out now closes **itself** (`getCurrentWindow().close()` — always reliable, no round-trip) and tags the close with a `redock` flag via the already-working `rdp:popout-closed` event. Main has one listener now: `redock:true` → `redockRdp` (reattach stream to main + drop the placeholder); plain/native-X close → end the session. `redockRdp` no longer closes the window (it's gone) and tolerates a reattach error; the backend session is webview-independent so closing the window never tears it down. Removed the `rdp:request-redock` listener + `redockGuard`. tsc + vite green.

## Latest — Fix #2: pop-out wouldn't close — missing `allow-destroy` capability

The redock returned the tab but the pop-out window stayed open & frozen, and re-popping focused that stale window (label `rdp-${sid}` collision → new-window create silently no-ops). Root cause: `capabilities/default.json` granted `core:window:allow-close` but **not `core:window:allow-destroy`** — so `redockRdp`'s `w.destroy()` was permission-denied and swallowed by the try/catch. (The clicks-appear-on-redock symptom was the cascade: stale frozen window got focus, its input still reached the backend, frames rendered to the main tab.) Fix: add `core:window:allow-destroy` to the capability + defensively `destroy()` any lingering same-label window before creating a new pop-out. **Requires a Rust rebuild** (capabilities are compiled in). tsc + vite green.

## Latest — Fix: RDP pop-out "return to tab" (X) hung the window / lost the session

Bug: clicking the pop-out's X froze the window and the session didn't re-dock. Root cause: the X closed its **own** window (`getCurrentWindow().close()`) while the backend was still streaming frames into that window's Tauri `Channel`. The backend forwarder (`rdp_session.rs`) breaks + **removes the session** the moment a `Channel.send` fails, so closing mid-delivery (a) wedged the webview and (b) raced `redockRdp`'s reattach → the session was torn down before it could be re-homed.

Fix (frontend only): the X no longer closes itself. It emits `rdp:request-redock {sid}`; the **main window** handles it by reattaching the stream to itself FIRST (backend sink swaps to main, stops streaming into the pop-out), THEN force-`destroy()`s the pop-out window. A `redockGuard` set suppresses the native-X "popout-closed → end session" handler for the close we trigger ourselves. Native window-X still ends the session as before. `RdpPopoutApp.tsx` + `store/index.ts`; tsc + vite green. Awaiting user verification.

## Latest — RDP floating "shield" bar: draggable + Minimize + Close-to-tab

The connection bar (`RdpViewport.tsx`) is now a movable shield. Drag it by the **host name** (`onBarPointerDown` on `.barTitle`, window-level pointermove/up) anywhere in the viewport; on release it **snaps to the nearest perimeter anchor** (3 cols × 2 rows: tl/tc/tr/bl/bc/br) via `barAnchor` + `.pos_*` CSS. While dragging it stays fully visible (inline left/top); resting placement is the snapped class. Auto-reveal zone now **follows the anchor** (top edge for top anchors, bottom edge for bottom ones) so a bar dragged to the bottom reveals where it sits.

Buttons: **Minimize** (`Minus` → `onMinimize`; in the pop-out window minimizes the OS window), Fullscreen (existing), Pop-out (existing, main window only), **Close→return-to-tab** (`X` → `onPopIn`, red hover; pop-out only — re-docks the session into the in-app tab via the existing `rdp:request-redock` → `redockRdp` path). i18n: `session.moveBar`/`session.minimize`/`session.redock` (en+ru). Design-clean: hairline + popover shadow, tokens only, mono host label, accent untouched. tsc + vite green.

## Latest — RDP crash fixed: rustls 0.23 CryptoProvider not selected

The post-negotiation crash (surfaced once the laptop accepted TLS) was a classic **rustls 0.23 panic**: `ClientConfig::builder()` needs a *process-default* `CryptoProvider`, and we never installed one (no `install_default`/`builder_with_provider` anywhere; rustls 0.23.40 via tokio-rustls 0.26). With both aws-lc-rs and ring potentially in the tree it can't auto-pick → panic. `tls_upgrade` only ran for the first time now (earlier the X.224 negotiation always failed first), which is why it was latent. Stage display confirmed: crash during the Connecting/TLS-upgrade phase, before Authenticating.

Fix (`rh-rdp/actor.rs::tls_upgrade`): build the rustls config with `builder_with_provider(Arc::new(rustls::crypto::aws_lc_rs::default_provider())).with_safe_default_protocol_versions()?` — explicit provider, no global install needed. The prior worker `catch_unwind` (panic=unwind) stays as a safety net. Backend; user compiles. Next: build + retry; if it connects past TLS we hit auth (Microsoft-account creds; may need `autologon` if the remote logon screen shows).

## Latest — RDP: TLS now reached (laptop reconfigured); worker no longer crashes the app

Progress: after setting `SecurityLayer=1` (Negotiate) on the laptop, the X.224 failure is gone — the target now offers **TLS** (mstsc shows its self-signed cert `CN=MSI-1`, "untrusted CA"; the runtime log confirmed `enable_tls=true` and the earlier `FailureCode(2)` = SSL_NOT_ALLOWED_BY_SERVER was purely the laptop's Standard-only config). So the SSL-advertise fix was correct; we're now PAST negotiation.

New problem: the client **crashes** in the now-reached post-negotiation path (TLS upgrade / `connect_finalize` / active loop) — code that never ran against this target before. Because release was `panic = "abort"`, any worker panic aborted the whole process.

Resilience fix (`crates/rh-rdp/src/actor.rs` + root `Cargo.toml`): release profile → `panic = "unwind"`, and the `rdp-session` worker thread body is wrapped in `std::panic::catch_unwind` → a panic becomes `RdpSessionEvent::Error + Closed(Error)` (clean UI error) instead of killing the app; the panic hook still writes `panic.log` before unwinding. `tls_upgrade` itself is clean (all `?`/`map_err`, no unwrap), so the panic is downstream. **Root cause pending** the user's `panic.log` + `app.log` tail from the crashing attempt. Backend; user compiles.

## Latest — RDP negotiation diagnostic log (cert prompt ⇒ target IS on TLS)

User reports mstsc shows a **certificate** trust prompt for that laptop — which means the target speaks **TLS** (Standard RDP Security has no X.509 cert), with **NLA off**. So it is NOT Standard-only; the `enable_tls: true` SSL-advertise fix from the prior entry *should* let us negotiate TLS like mstsc. Since the user still saw the identical error, the likeliest cause is a **stale build** (the tested binary predated the fix). Added a runtime log in `rh-rdp/actor.rs::connect` (captured `adv_tls`/`adv_credssp` *before* `config` is moved into the connector — avoids use-after-move): logs `enable_tls`/`enable_credssp` before X.224 negotiation, and a line after it succeeds. Next run is conclusive: `enable_tls=true` + still "Standard RDP Security" ⇒ target truly offers neither SSL nor CredSSP (→ FreeRDP); `enable_tls=false` ⇒ stale build. Backend; user does a **clean** rebuild + retry and reports the log line.

## Latest — RDP now advertises SSL too (connect to NLA-disabled targets like mstsc)

**Symptom:** a local laptop that **mstsc connects to fine** failed in our client with `negotiation failure: server only supports Standard RDP Security`.

**Root cause (confirmed via IronRDP docs/maintainers):** IronRDP *deliberately* does not implement legacy "Standard RDP Security" (RC4) — `is_standard_rdp_security()` → hard error; the maintainers call it broken/insecure and point to FreeRDP for old stuff. mstsc supports it, we don't. **But** the real trigger on our side: `build_config` had `enable_tls: false` + `enable_credssp: true`, so we advertised **CredSSP/HYBRID (NLA) only**. A target with NLA *disabled* (Windows Home + RDP Wrapper, or "Negotiate" security layer) can't satisfy HYBRID and falls back to Standard → IronRDP refuses. Per the IronRDP source, SSL is only advertised when `enable_tls` is set.

**Fix:** `enable_tls: true` in `build_config` (`crates/rh-rdp/src/actor.rs`) so we advertise **SSL + HYBRID**. NLA-disabled targets can now pick TLS-without-NLA (what mstsc does); CredSSP stays preferred where supported, so existing NLA servers are unaffected. We still drive the TLS upgrade ourselves (`enable_tls` only changes the advertised X.224 protocols, not who upgrades) — so the manual `connect_begin → tls_upgrade → mark_as_upgraded → connect_finalize` flow is unchanged.

**SPIKE / unverified:** this `enable_tls` combo wasn't exercised before; user compiles + tests against the laptop. Watch for: (a) SSL-without-NLA showing the remote *interactive* logon screen instead of auto-login — if so, set `autologon: true` (sends creds in the Client Info PDU); (b) a genuinely Standard-only target (no TLS at all) will STILL fail — then the only path is the documented **FreeRDP fallback** (big), since IronRDP will never do Standard RDP Security. My earlier "reconfigure the server" advice was wrong for the mstsc-works case — corrected.
